                                ...
                                <tr key={o.id} className="hover:bg-gray-50 transition-colors relative">
                                    ...
                                    <td className="px-6 py-4 relative">
                                        ...
                                        <div className="relative">
                                            ...
                                            {/* Dropdown Menu */}
                                            {activeDropdown === o.id && (
                                                <div className="absolute top-full left-0 mt-2 w-40 bg-white rounded-lg shadow-xl border border-gray-100 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                                    {statusOptions.map((opt) => (
                                                        <button
                                                            key={opt}
                                                            onClick={() => handleStatusChange(o.id, opt)}
                                                            className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 flex items-center gap-2 ${o.status === opt ? 'font-bold text-[#ff6a00] bg-orange-50' : 'text-gray-600'}`}
                                                        >
